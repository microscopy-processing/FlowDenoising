...
def get_flower_CPU(l=OF_LEVELS, w=OF_WINDOW_SIZE):
    NT = namedtuple("flower", "l w")
    flower = NT(l, w)
    return flower

def get_flow_with_prev_flow_CPU(
        flower, reference, target, prev_flow=None):
    if __debug__:
        time_0 = time.perf_counter()
...
def get_flower_GPU(l=OF_LEVELS, w=OF_WINDOW_SIZE):
...
def get_flow_with_prev_flow_GPU(flower, reference, target, prev_flow=None):
    if __debug__:
        time_0 = time.perf_counter()
...
def get_flow_without_prev_flow_GPU(flower, reference, target, prev_flow=None):
    if __debug__:
        time_0 = time.perf_counter()
    GPU_target = cv2.cuda_GpuMat()
    GPU_target.upload(target)
...
class FlowDenoising(GaussianDenoising):
    ...
    def __init__(self, number_of_processes, kernels, l, w, get_flower, get_flow, warp_slice):
        super().__init__(number_of_processes, kernels)
        self.l = l
        self.w = w
        self.get_flow = get_flow
        self.warp_slice = warp_slice
        self.get_flower = get_flower
    ...
    def filter_along_Z_slice(self, z, kernel):
        ...
        flower = self.get_flower(l, w)
        for i in range(ks2 - 1, -1, -1):
            flow = self.get_flow(flower,
                                 self.vol[(z + i - ks2) % self.vol.shape[0], :, :],
                                 self.vol[z, :, :], prev_flow)
            prev_flow = flow
    ...
    def filter_along_Y_slice(self, y, kernel):
        ...
        for i in range(ks2 - 1, -1, -1):
            flow = self.get_flow(flower,
                                 self.vol[:, (y + i - ks2) % self.vol.shape[1], :],
                                 self.vol[:, y, :], prev_flow)
            prev_flow = flow
    ...
    def filter_along_X_slice(self, x, kernel):
        ...
        for i in range(ks2 - 1, -1, -1):
            flow = self.get_flow(flower,
                                 self.vol[:, :, (x + i - ks2) % self.vol.shape[2]],
                                 self.vol[:, :, x], prev_flow)
            prev_flow = flow
...
if __name__ == "__main__":    
    ...
    if args.no_OF:
        ...
        fd = FlowDenoising(number_of_processes, kernels, l, w, get_flower, get_flow, warp_slice)
