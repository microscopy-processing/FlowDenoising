...
def no_OF_filter_along_Z_slice(vol, filtered_vol, z, kernel):
    ks2 = kernel.size//2
...
def no_OF_filter_along_Y_slice(vol, filtered_vol, y, kernel):
    ks2 = kernel.size//2
...
def no_OF_filter_along_X_slice(vol, filtered_vol, x, kernel):
    ks2 = kernel.size//2
...
def no_OF_filter_along_Y_chunk(vol, filtered_vol, chunk_index, chunk_size, chunk_offset, kernel):
    for y in range(chunk_size):
        no_OF_filter_along_Y_slice(vol, filtered_vol, chunk_index*chunk_size + y + chunk_offset, kernel)
    return chunk_index

def no_OF_filter_along_X_chunk(vol, filtered_vol, chunk_index, chunk_size, chunk_offset, kernel):
    for x in range(chunk_size):
        no_OF_filter_along_X_slice(vol, filtered_vol, chunk_index*chunk_size + x + chunk_offset, kernel)
    return chunk_index

def no_OF_filter_along_Z(vol, filtered_vol, kernel):
...
def no_OF_filter_along_Y(vol, filtered_vol, kernel):
...
def no_OF_filter_along_X(vol, filtered_vol, kernel):
...
def no_OF_filter(vol, filtered_vol, kernels):
    no_OF_filter_along_Z(vol, filtered_vol, kernels[0])
    vol[...] = filtered_vol[...]
    no_OF_filter_along_Y(vol, filtered_vol, kernels[1])
    vol[...] = filtered_vol[...]
    no_OF_filter_along_X(vol, filtered_vol, kernels[2])
...
if __name__ == "__main__":    
    ...
    if args.no_OF:
        no_OF_filter(vol, filtered_vol, kernels)
    else:
