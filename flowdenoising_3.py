...
def no_OF_filter_along_Z(vol, kernel, mean):
    ...
    for z in range(Z_dim):
        ...
        for i in range(kernel.size):
            ...
            #tmp_slice += __padded_vol__[z + i, kernel.size//2:shape_of_vol[1] + kernel.size//2, kernel.size//2:shape_of_vol[2] + kernel.size//2]*kernel[i]
        filtered_vol[z, :, :] = tmp_slice
...
def no_OF_filter_along_Y(vol, kernel, mean):
    ...
    for y in range(Y_dim):
        ...
        for i in range(kernel.size):
            ...
            #tmp_slice += __padded_vol__[kernel.size//2:shape_of_vol[0] + kernel.size//2, y + i, kernel.size//2:shape_of_vol[2] + kernel.size//2]*kernel[i]
        filtered_vol[:, y, :] = tmp_slice
...
def no_OF_filter_along_X(vol, kernel, mean):
    ...
    for x in range(X_dim):
        ...
        for i in range(kernel.size):
            ...
            #print(padded_vol[:, :, x + i][:10])
            #print(__padded_vol__[kernel.size//2:-kernel.size//2, kernel.size//2:-kernel.size//2, x + i][:10])
            #tmp_slice += __padded_vol__[kernel.size//2:shape_of_vol[0] + kernel.size//2, kernel.size//2:shape_of_vol[1] + kernel.size//2, x + i]*kernel[i]
        filtered_vol[:, :, x] = tmp_slice
        __percent__ = int(100*(x/X_dim))
    if __debug__:
        time_1 = time.process_time()
...
if __name__ == "__main__":
    ...
    __padded_vol__[kernels[0].size//2:vol.shape[0] + kernels[0].size//2,
                   kernels[1].size//2:vol.shape[1] + kernels[1].size//2,
                   kernels[2].size//2:vol.shape[2] + kernels[2].size//2] = vol

    if args.no_OF:
        filtered_vol = no_OF_filter(vol, kernels)
    else:
        filtered_vol = OF_filter(vol, kernels, l, w)
